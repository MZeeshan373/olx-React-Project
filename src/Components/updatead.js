...
class UpdateAd extends Component {
    ...
    handleUpload = (e) => {
        if (e.target.files[0]) {
            ...
            this.setState({ image });
            this.setState({ url: '' })
            const uploadTask = storage.ref(`images/${randomNum}${image.name}`).put(image)
            uploadTask.on('state_changed',
                ...
                () => {
                    // complete funcion
                    storage.ref('images').child(randomNum + image.name).getDownloadURL()
                        .then(url => {
                            this.setState({ url });
                            console.log(url)
                        })
    ...
    check = () => {
        ...
        if (this.refs.catagory.value === "Mobile") {
            ...
            this.setState({ brand: ['Select Catagory First'] })
        }
        ...
        setTimeout(() => {
            fetch('/getDetail?_id=' + this.props.match.params.id)
                ...
                .then((data) => {
                    ...
                    this.setState({ url: JSON.parse(data).image })
                    this.refs.description.value = this.refs.description.value + JSON.parse(data).description;
    ...
    render() {
        ...
        return (
            <div className='ad-form'>
                <form>
                    <fieldset>
                        <legend>Update Ad Details</legend>
                        ...
                        <div className="form-group">
                            ...
                            <input type="text" className="form-control" defaultValue={this.state.ad.title} id="title" placeholder="Enter product Title here" ref='title' />
                        </div>
                        <div className="form-group">
                            <label htmlFor="price">Set a price</label>
                            <input type="number" className="form-control" defaultValue={this.state.ad.price} id="price" placeholder="Enter product price here in PKR" ref='price' />
                        </div>
                        <div className="form-group">
                        ...
                        <img ref='img' src={this.state.url || this.state.ad.image || "https://dummyimage.com/60x60/c7bfc7/0011ff.jpg"} style={{ width: '60px', height: '60px' }} alt="Uploaded" />
                        ...
                        <br />
                        <button type='button' onClick={this.updatead} className="btn btn-secondary">Post Ad</button>
                    </fieldset>
...
export default connect(mapStateToProps)(UpdateAd);